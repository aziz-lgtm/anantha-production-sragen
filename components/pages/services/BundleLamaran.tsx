...
import { Frame, Plus, Minus } from "lucide-react";
...
import {
  ...
  getDiscount,
  type Product,
} from "@/data/services/bundle-lamaran";

type CartLine = { product: Product; qty: number };

export default function BundleLamaranContent() {
  ...
  const { amount: discount, label: discountLabel } = getDiscount(subtotal, referralValid);
  ...
  const handleSubmit = () => {
    ...
    const message = [
      "Halo, saya ingin memesan paket Bundle Lamaran.",
      "",
      `Nama: ${name}`,
      `No. HP: ${phone}`,
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-20 md:px-10">
      <div className="max-w-2xl">
        ...
        >
          {popBundleLamaran.title}
        </h1>
        <p
          className="mt-4 text-[1.05rem] leading-[1.7] text-[#cdd3c8]/80"
          style={{ fontFamily: "'Manrope', system-ui, sans-serif" }}
        >
          {popBundleLamaran.description}
        </p>
      </div>

      {/* product grid -- each item sits inside a picture-frame style box.
          Swap the Frame icon for a real photo (next/image) per product
          once you have assets; the frame border is designed to still look
          intentional either way, as a stylistic border around the photo. */}
      <div className="mt-12 grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-4">
        {products.map((product) => {
          ...
          return (
            ...
            >
              <div className="relative flex h-32 items-center justify-center rounded-xl border-[3px] border-double border-[#c6a15b]/50 bg-[#0b1410]">
                <Frame className="h-9 w-9 text-[#c6a15b]" strokeWidth={1.3} />
              </div>
              <h3
              ...
              <p className="mt-1 text-xs text-[#cdd3c8]/70">{product.layanan}</p>
