...
import {
  ...
  getDiscount,
  type Product,
} from "@/data/services/custom-property";

type CartLine = { product: Product; qty: number };

export default function CustomPropertyContent() {
  ...
  const handleSubmit = () => {
    ...
    const message = [
      "Halo, saya ingin memesan Custom Property.",
      "",
      `Nama: ${name}`,
      `No. HP: ${phone}`,
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-20 md:px-10">
      <div className="max-w-2xl">
        ...
        >
          {popCustomProperty.title}
        </h1>
        <p
          className="mt-4 text-[1.05rem] leading-[1.7] text-[#cdd3c8]/80"
          style={{ fontFamily: "'Manrope', system-ui, sans-serif" }}
        >
          {popCustomProperty.description}
        </p>
      </div>

      {/* product grid -- swap the Box icon for a real product photo
          ...
          return (
            ...
            >
              <div className="flex h-28 items-center justify-center rounded-xl bg-[#0b1410]">
                <Box className="h-10 w-10 text-[#c6a15b]" strokeWidth={1.5} />
              </div>
              <h3
